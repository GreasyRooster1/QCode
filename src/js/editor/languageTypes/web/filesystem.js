...
class Filesystem {
    constructor(defaultFileName) {
        ...
        this.defaultFile = this.system["/"][defaultFileName];
    ...
    findFilePath(folder, id, path) {
        // @ts-ignore
        for (let [key, frag] of Object.entries(folder)) {
            if (isFolder(frag)) {
                let out = this.findFilePath(frag, id, path + "/" + key);
                if (out !== null) {
                    return out;
                }
            }
    ...
    serializeFolder(folder, jsonObject) {
        // @ts-ignore
        for (let [key, frag] of Object.entries(folder)) {
            if (isFolder(frag)) {
                jsonObject[key] = {};
...
function getFolderDom(key, folder) {
...
function hasFileIcon(extension) {
    let allowedExtensions = ["css", "html", "ico", "jar", "jpg", "js", "mp3", "obj", "png", "py", "svg", "sys", "txt"];
    return allowedExtensions.indexOf(extension) !== -1;
}
export { Filesystem, isFolder, FilesystemFile, getFolderDom, cleanFileName };
